...
const HodNotification = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (storedToken !== null) {
          const response = await axios.post('http://192.168.200.150:5000/api/hod/verifytokens', { token: storedToken });
          setHodDepartment(response.data.department);
        } else {
  ...
  useEffect(() => {
    const fetchUpdates = async () => {
      try {
        ...
        // Filter updates based on hodDepartment
        const filteredUpdates = response.data.filter(update => update.department === hodDepartment);
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.heading}>Daily Updates for {hodDepartment}</Text>
      {updates.map((update) => (
...
const styles = StyleSheet.create({
  ...
  text: {
    fontSize: 16,
    marginBottom: 10,
    color: '#555', // Change text color to dark gray
  },
});
