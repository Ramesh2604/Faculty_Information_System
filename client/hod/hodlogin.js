...
import { View, TextInput, Button, StyleSheet, Alert,ImageBackground,StatusBar,Text } from 'react-native';
import AsyncStorage from '@react-native-async-storage/async-storage';
export default function Hodlogin({ navigation }) {
  ...
  const handleLogin = async () => {
    try {
      const response = await fetch('http://192.168.200.150:5000/api/hod/login', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        AsyncStorage.setItem("Hodtoken" ,data.token);
        // Check if user exists and has the role 'hod'
        if (data.user && data.user.role === "hod") {
          Alert.alert('Success', 'Login successful', [
            { text: 'OK', onPress: () => navigation.navigate('HodHome') },
          ]);
        } else {
    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.overlay}>
          ...
          <Text style={styles.head}>Hod Login</Text>
      <TextInput
        style={styles.input}
        placeholder="Email"
        value={email}
